...
export function OutstandingAdvancesPage() {
  ...
  const items = useMemo<ExpenseAdvanceRequestDto[]>(
    () => (requestsQ.data?.items ?? []).filter((x: ExpenseAdvanceRequestDto) => x.outstandingAmount > 0),
  ...
  return (
    <div className="page-grid">
      <section className="panel">
        <div className="section-heading"><h2>Outstanding Advances</h2><span className="muted">{items.length} item(s)</span></div>
        <div className="muted">Outstanding advances awaiting full retirement, refund, or recovery.</div>
