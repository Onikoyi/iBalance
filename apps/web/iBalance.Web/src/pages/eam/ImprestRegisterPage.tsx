...
export function ImprestRegisterPage() {
  ...
  const items = useMemo<ExpenseAdvanceRequestDto[]>(
    () => (requestsQ.data?.items ?? []).filter((x: ExpenseAdvanceRequestDto) => x.advanceTypeId === '44444444-4444-4444-4444-444444444444'),
  ...
  return (
    <div className="page-grid">
      <section className="panel">
        <div className="section-heading"><h2>Imprest Register</h2><span className="muted">{items.length} item(s)</span></div>
        <div className="muted">Monitor imprest requests, outstanding balances, and retirement exposure.</div>
