...
export function PurchaseOrdersPage() {
  ...
  const canManage = canCreatePurchaseOrders();
  const canApprove = canApprovePurchaseOrders();
  const canReject = canRejectPurchaseOrders();
  ...
  const createFromReqMut = useMutation({
    mutationFn: ({ requisitionId, payload }: { requisitionId: string; payload: any }) =>
      createPurchaseOrderFromRequisition(requisitionId, payload),
    ...
    onError: (error) =>
      setErrorText(getTenantReadableError(error, 'Unable to create purchase order from requisition.')),
  });
  ...
  const rejectMut = useMutation({
    mutationFn: ({ purchaseOrderId, reason }: { purchaseOrderId: string; reason: string }) =>
      rejectPurchaseOrder(purchaseOrderId, reason),
    onSuccess: async (response: any) => {
      await qc.invalidateQueries({ queryKey: ['procurement-purchase-orders'] });
  ...
  const approvedRequisitions = ((requisitionsQ.data?.items ?? []) as PurchaseRequisitionDto[]).filter(
    (item) => item.status === 3
  );
  ...
  async function createFromApprovedRequisition() {
    ...
    if (!canManage) {
      setErrorText('You do not have permission to create purchase orders.');
      return;
    }

    if (!selectedRequisitionId) {
      setErrorText('Approved requisition is required.');
      return;
    }
  ...
  if (!canView) {
    return <div className="panel error-panel">You do not have access to Purchase Orders.</div>;
  }

  if (purchaseOrdersQ.isLoading) {
    return <div className="panel">Loading purchase orders...</div>;
  }

  if (purchaseOrdersQ.isError || !purchaseOrdersQ.data) {
    return <div className="panel error-panel">Unable to load purchase orders.</div>;
  }
      ...
      <section className="panel">
        <div className="section-heading">
          <div>
            ...
            <div className="muted">
              Create, approve, issue, and monitor vendor purchase orders from approved requisitions.
            </div>
          </div>

          {canManage ? (
            <button className="button" type="button" onClick={() => setShowCreate((value) => !value)}>
              {showCreate ? 'Close' : 'New PO from Requisition'}
            </button>
          ) : null}
        </div>
        ...
        {requisitionsQ.isError ? (
          <div className="panel" style={{ marginTop: 12 }}>
            <div className="muted">
              Approved requisitions could not be loaded. Existing purchase orders are still available, but creating a new purchase order from requisition is temporarily unavailable.
            </div>
          </div>
        ) : null}
        ...
        {vendorsQ.isError ? (
          <div className="panel" style={{ marginTop: 12 }}>
            <div className="muted">
              Vendors could not be loaded. Existing purchase orders are still available, but vendor selection for new purchase orders is temporarily unavailable.
            </div>
          </div>
        ) : null}
          ...
          <div className="panel" style={{ marginTop: 12 }}>
            ...
            <div className="form-grid three">
              <div className="form-row">
                ...
                <select
                  ...
                  disabled={requisitionsQ.isLoading || requisitionsQ.isError}
                >
                  <option value="">Select approved requisition</option>
                  {approvedRequisitions.map((item) => (
                    <option key={item.id} value={item.id}>
                      {item.requisitionNumber} - {item.purpose}
                    </option>
                  ))}
                </select>
              </div>
              ...
              <div className="form-row">
                ...
                <select
                  ...
                  disabled={vendorsQ.isLoading || vendorsQ.isError}
                >
              ...
              <div className="form-row">
                ...
                <input
                  ...
                  type="date"
                  value={orderDateUtc}
                  onChange={(e) => setOrderDateUtc(e.target.value)}
                />
              </div>
              ...
              <div className="form-row">
                ...
                <input
                  className="input"
                  type="date"
                  value={expectedDeliveryUtc}
                  onChange={(e) => setExpectedDeliveryUtc(e.target.value)}
                />
              </div>
            </div>
            ...
            <div className="inline-actions">
              <button
                ...
                type="button"
                onClick={createFromApprovedRequisition}
                disabled={
                  createFromReqMut.isPending ||
              ...
              {purchaseOrders.length === 0 ? (
                ...
                purchaseOrders.map((item) => (
                  <tr key={item.id}>
                    ...
                    <td>
                      <div className="inline-actions">
                        <button className="button" type="button" onClick={() => setSelectedPurchaseOrderId(item.id)}>
                          View
                        </button>
                        <button
                          className="button secondary"
                          type="button"
                          disabled={!canManage || item.status !== 1 || submitMut.isPending}
                        ...
                        <button
                          className="button secondary"
                          type="button"
                          disabled={!canApprove || item.status !== 2 || approveMut.isPending}
        ...
        <section className="panel">
          <div className="section-heading">
            <div>
              ...
              <div className="muted">
                {selectedPurchaseOrder.purchaseOrderNumber} · {statusLabel(selectedPurchaseOrder.status)}
              </div>
            </div>
          </div>
          ...
          <div className="kv">
            <div className="kv-row">
              <span>Vendor</span>
              <span>{selectedPurchaseOrder.vendorName || '—'}</span>
            </div>
            <div className="kv-row">
              <span>Requisition</span>
              <span>{selectedPurchaseOrder.purchaseRequisitionNumber || '—'}</span>
            </div>
            <div className="kv-row">
              <span>Expected Delivery</span>
              <span>{formatDate(selectedPurchaseOrder.expectedDeliveryUtc)}</span>
            </div>
            <div className="kv-row">
              <span>Currency</span>
              <span>{selectedPurchaseOrder.currencyCode}</span>
            </div>
            <div className="kv-row">
              <span>Notes</span>
              <span>{selectedPurchaseOrder.notes || '—'}</span>
            </div>
          </div>
          ...
          <div className="form-grid two" style={{ marginTop: 12 }}>
            ...
            <div className="form-row">
              ...
              <button
                ...
                type="button"
                disabled={!canReject || selectedPurchaseOrder.status !== 2 || !rejectReason.trim()}
                onClick={() =>
                  rejectMut.mutate({
