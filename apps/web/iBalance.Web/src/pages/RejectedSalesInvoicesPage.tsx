...
import {
  ...
  updateRejectedSalesInvoice,
  type RejectedSalesInvoiceDto,
...
const emptyLine: SalesInvoiceLineDto = {
  description: '',
  quantity: 1,
  unitPrice: 0,
};

const emptyForm: UpdateSalesInvoiceRequest = {
  customerId: '',
  invoiceDateUtc: '',
  invoiceNumber: '',
  description: '',
...
export function RejectedSalesInvoicesPage() {
  const qc = useQueryClient();
  const canView = canViewAccountsReceivable();
  const canManage = canCreateSalesInvoices();
  const canSubmitApproval = canSubmitSalesInvoices();

  const [selectedInvoice, setSelectedInvoice] = useState<RejectedSalesInvoiceDto | null>(null);
  const [form, setForm] = useState<UpdateSalesInvoiceRequest>(emptyForm);
  ...
  const rejectedInvoicesQ = useQuery({
    queryKey: ['ar-rejected-sales-invoices'],
    queryFn: getRejectedSalesInvoices,
    enabled: canView,
  });
  ...
  async function refresh() {
    await qc.invalidateQueries({ queryKey: ['ar-rejected-sales-invoices'] });
  ...
  const updateMut = useMutation({
    mutationFn: () => {
      if (!selectedInvoice) {
        throw new Error('No rejected sales invoice selected.');
      }

      return updateRejectedSalesInvoice(selectedInvoice.id, {
    ...
    onSuccess: async () => {
      ...
      setForm(emptyForm);
      setErrorText('');
      setInfoText('Rejected sales invoice updated successfully. It remains rejected until resubmitted.');
    },
    onError: (error) => {
      setErrorText(getTenantReadableError(error, 'Unable to update rejected sales invoice.'));
      setInfoText('');
    },
  });
  ...
  const submitMut = useMutation({
    ...
    onSuccess: async () => {
      await refresh();
      setErrorText('');
      setInfoText('Sales invoice resubmitted for approval successfully.');
    },
    onError: (error) => {
      setErrorText(getTenantReadableError(error, 'Unable to resubmit sales invoice for approval.'));
      setInfoText('');
    },
  });

  const deleteMut = useMutation({
    mutationFn: (salesInvoiceId: string) => deleteRejectedSalesInvoice(salesInvoiceId),
    onSuccess: async () => {
      await refresh();
      setErrorText('');
      setInfoText('Rejected sales invoice deleted successfully.');
    },
    onError: (error) => {
      setErrorText(getTenantReadableError(error, 'Unable to delete rejected sales invoice.'));
      setInfoText('');
    },
  });
  ...
  const netReceivableAmount = taxPreviewQ.data?.netAmount ?? invoiceBaseAmount;

  function update<K extends keyof UpdateSalesInvoiceRequest>(key: K, value: UpdateSalesInvoiceRequest[K]) {
    setForm((state) => ({ ...state, [key]: value }));
  }

  function updateLine(index: number, key: keyof SalesInvoiceLineDto, value: string | number) {
    setForm((state) => {
  ...
  function openEdit(invoice: RejectedSalesInvoiceDto) {
    if (!canManage) {
      setErrorText('You do not have permission to edit rejected sales invoices.');
      setInfoText('');
      return;
    }
  ...
  async function submitUpdate() {
    ...
    if (!selectedInvoice) {
      setErrorText('Please select a rejected sales invoice to edit.');
      return;
    }
    ...
    if (form.lines.length === 0) {
      setErrorText('At least one sales invoice line is required.');
      return;
    }

    for (const line of form.lines) {
      if (!line.description.trim()) {
        setErrorText('Each sales invoice line must have a description.');
        return;
      }

      if (Number(line.quantity || 0) <= 0) {
        setErrorText('Each sales invoice line quantity must be greater than zero.');
        return;
      }

      if (Number(line.unitPrice || 0) < 0) {
        setErrorText('Sales invoice line unit price cannot be negative.');
        return;
      }
    }
  ...
  async function submitForApproval(invoice: RejectedSalesInvoiceDto) {
    setErrorText('');
    setInfoText('');

    if (!canSubmitApproval) {
      setErrorText('You do not have permission to submit rejected sales invoices for approval.');
      return;
    }
  ...
  async function deleteInvoice(invoice: RejectedSalesInvoiceDto) {
    setErrorText('');
    setInfoText('');

    if (!canManage) {
      setErrorText('You do not have permission to delete rejected sales invoices.');
      return;
    }

    const confirmed = window.confirm(
      `Delete rejected sales invoice ${invoice.invoiceNumber}? This cannot be undone.`
    );
  ...
  if (!canView) {
    return <div className="panel error-panel">You do not have access to view rejected sales invoices.</div>;
  }

  if (rejectedInvoicesQ.isLoading || customersQ.isLoading) {
    return <div className="panel">Loading rejected sales invoices...</div>;
  }
  ...
  if (
    ...
    return <div className="panel error-panel">We could not load rejected sales invoices at this time.</div>;
  }
    ...
    <div className="page-grid">
      <section className="panel">
        <div className="section-heading">
          <div>
            <h2>Rejected Sales Invoices</h2>
            <div className="muted">
              Correct rejected customer invoices, resubmit them for approval, or delete invoices no longer needed.
            </div>
          </div>
        ...
        <div className="form-row" style={{ marginBottom: 16 }}>
          ...
          <input
            ...
            placeholder="Invoice number, customer, description, rejection reason"
          />
        </div>
              ...
              {filteredInvoices.length === 0 ? (
                <tr>
                  <td colSpan={11} className="muted">
                    No rejected sales invoices matched your search.
                  </td>
                </tr>
              ) : (
                filteredInvoices.map((invoice) => (
                  <tr key={invoice.id}>
                    ...
                    <td>{salesInvoiceStatusLabel(invoice.status)}</td>
                    ...
                    <td style={{ textAlign: 'right' }}>{formatAmount(invoice.netReceivableAmount || invoice.totalAmount)}</td>
        ...
        <div className="modal-backdrop" onMouseDown={closeEdit}>
          <div className="modal" onMouseDown={(e) => e.stopPropagation()}>
            <div className="modal-header">
              <h2>Edit Rejected Sales Invoice</h2>
            ...
            <div className="kv" style={{ marginBottom: 16 }}>
              <div className="kv-row"><span>Current Status</span><span>{salesInvoiceStatusLabel(selectedInvoice.status)}</span></div>
            ...
            <div className="section-heading" style={{ marginTop: 16 }}>
              <div>
                ...
                <span className="muted">Select setup-driven sales tax codes for this invoice</span>
              </div>
            </div>
            ...
            <div className="panel" style={{ marginBottom: 16 }}>
              {taxCodesQ.isLoading ? (
                ...
                <div className="muted">
                  Tax codes could not be loaded. You can still review and edit the rejected invoice, but tax-code adjustment is unavailable right now.
                </div>
            ...
            <div className="kv" style={{ marginBottom: 16 }}>
              ...
              <div className="kv-row"><span>Net Receivable Amount</span><span>{formatAmount(netReceivableAmount)}</span></div>
            </div>
