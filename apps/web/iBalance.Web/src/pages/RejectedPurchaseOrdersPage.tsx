...
import { canCreatePurchaseOrders, canViewProcurement } from '../lib/auth';
...
export function RejectedPurchaseOrdersPage() {
  ...
  const rejectedQ = useQuery({
    queryKey: ['procurement-purchase-orders-rejected'],
    queryFn: getRejectedPurchaseOrders,
    enabled: canView,
  });
  ...
  const updateMut = useMutation({
    mutationFn: ({ purchaseOrderId, request }: { purchaseOrderId: string; request: UpdatePurchaseOrderRequest }) =>
      updatePurchaseOrder(purchaseOrderId, request),
  ...
  async function save() {
    ...
    if (!canManage) {
      setErrorText('You do not have permission to edit rejected purchase orders.');
      return;
    }
  ...
  if (!canView) {
    return <div className="panel error-panel">You do not have access to Rejected Purchase Orders.</div>;
  }

  if (rejectedQ.isLoading) {
    return <div className="panel">Loading rejected purchase orders...</div>;
  }

  if (rejectedQ.isError || !rejectedQ.data) {
    return <div className="panel error-panel">Unable to load rejected purchase orders.</div>;
  }
      ...
      <section className="panel">
        ...
        {itemsQ.isError ? (
          <div className="panel" style={{ marginTop: 12 }}>
            <div className="muted">
              Inventory items could not be loaded. Rejected purchase orders can still be viewed, but item-assisted editing is temporarily limited.
            </div>
          </div>
        ) : null}
        ...
        {vendorsQ.isError ? (
          <div className="panel" style={{ marginTop: 12 }}>
            <div className="muted">
              Vendors could not be loaded. Rejected purchase orders can still be viewed, but vendor reassignment is temporarily unavailable.
            </div>
          </div>
        ) : null}
                  ...
                  <tr key={item.id}>
                    ...
                    <td>
                      <div className="inline-actions">
                        <button className="button" type="button" onClick={() => setSelected(item)}>
                          Edit
                        </button>
                        <button
      ...
      {selected ? (
        <section className="panel">
          ...
          <div className="form-grid three">
              ...
              <select
                ...
                onChange={(e) =>
                  setSelected({
                    ...
                    vendorName: vendors.find((v) => v.id === e.target.value)?.vendorName || null,
                  })
                }
              >
            ...
            <div className="form-row">
              ...
              <input
                className="input"
                type="date"
                value={selected.orderDateUtc.slice(0, 10)}
                onChange={(e) => setSelected({ ...selected, orderDateUtc: dateInputToUtc(e.target.value) })}
              />
            </div>
            ...
            <div className="form-row">
              ...
              <input
                className="input"
                type="date"
                value={(selected.expectedDeliveryUtc || '').slice(0, 10)}
                onChange={(e) =>
                  setSelected({ ...selected, expectedDeliveryUtc: dateInputToUtc(e.target.value) })
                }
              />
            </div>
            ...
            <div className="form-row">
              ...
              <input
                ...
                onChange={(e) => setSelected({ ...selected, currencyCode: e.target.value.toUpperCase() })}
              />
            </div>
          </div>
              ...
              <div className="form-grid four">
                <div className="form-row">
                  ...
                  <select
                    ...
                    onChange={(e) => patchLine(index, { inventoryItemId: e.target.value })}
                  >
                ...
                <div className="form-row">
                  ...
                  <input
                    ...
                    onChange={(e) => patchLine(index, { unitPrice: Number(e.target.value) })}
                  />
                </div>
              </div>
          ...
          <div className="inline-actions" style={{ marginTop: 12 }}>
            <button className="button primary" type="button" onClick={save} disabled={updateMut.isPending}>
              Save Changes
            </button>
            <button
              ...
              onClick={() =>
                canManage
                  ? resubmitMut.mutate(selected.id)
                  : setErrorText('You do not have permission to resubmit purchase orders.')
              }
              disabled={resubmitMut.isPending || !canManage}
            >
              Resubmit
            </button>
          </div>
